...
const server = http.createServer(async (req, res) => {
  ...
  // API Routes
  // Handle POST /api/shorturls - Create short URL
  if (pathname === '/api/shorturls' && req.method === 'POST') {
    let body = '';
    req.on('data', chunk => {
      body += chunk.toString();
    });
  ...
  // GET /api/shorturls/stats - Get all shortened URLs and their statistics
  if (pathname === '/api/shorturls/stats' && req.method === 'GET') {
  ...
  // GET /api/shorturls/stats/:shortcode - Get stats for shortened URL
  if (pathname.startsWith('/api/shorturls/stats/') && req.method === 'GET') {
    const shortcode = pathname.split('/')[4];
  ...
  // Handle GET /:shortcode - Redirect to original URL (but not for static files)
  if (pathname.startsWith('/') && req.method === 'GET' && pathname !== '/' && 
...
server.listen(port, () => {
  console.log(`URL Shortener application running on http://localhost:${port}`);
  logger.logBackend('info', 'url-shortener', `URL Shortener application running on port ${port}`);
});